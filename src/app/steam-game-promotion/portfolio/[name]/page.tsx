...
const SingleGamePage = () => {
  ...
  const slug = params.name as string | null;
  ...
  useEffect(() => {
    if (!slug) {
      setLoading(false);
      setGame(null);
      setErrorMessage("No game slug provided");
      return;
    }
    ...
    const fetchGame = async () => {
      ...
      if (error) {
        ...
        setGame(null);
        setErrorMessage(error.message); // error.message is a string
      } else {
        setGame(data);
        setErrorMessage(undefined); // Reset error message on success
      }
      setLoading(false);
    };
  ...
  const descriptionHtml = game.description
    ? generateHTML(game.description as TiptapJson, [StarterKit])
